...
class CreateClass(CreateView):
    ...
    sucsess_url=reverse_lazy('list')

def logoutview(request):
    logout(request)
    return redirect('login')

def evaluatiomview(request,pk):
    post=ReviewModel.objects.get(pk=pk)
    author_name=request.user.get_username()+str(request.user.id)
    if author_name in post.useful_review_recoad:
        return redirect('list')
    else:
        post.useful_review=post.useful_review+1
        post.useful_review_recoad=post.useful_review_recoad+author_name
        post.save()
        return redirect('list')